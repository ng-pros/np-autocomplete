...
angular.module('templates-npAutocomplete', ['np-autocomplete-item-template.tpl.html', 'np-autocomplete-template.tpl.html']);

angular.module("np-autocomplete-item-template.tpl.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("np-autocomplete-item-template.tpl.html",
    "<button type=\"button\" class=\"list-group-item\" ng-repeat=\"item in searchResults\" ng-click=\"selectItem(item)\" ng-bind-html=\"match($eval('item.' + npAutocomplete.nameAttr))\"></button>");
}]);

angular.module("np-autocomplete-template.tpl.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("np-autocomplete-template.tpl.html",
    "<div id=\"np-autocomplete-input\"></div>\n" +
    "<div class=\"list-group np-autocomplete-list\" ng-if=\"isOpened\">\n" +
    "	<button type=\"button\" class=\"list-group-item\" ng-if=\"!searchResults.length && !isLoading\" ng-click=\"clear()\">{{noResultsMessage}}</button>\n" +
    "	<a class=\"list-group-item\" ng-if=\"isLoading\">{{loadingMessage}}</a>\n" +
    "</div>");
}]);
